...
// prisma is not used in client component; removed import
...
const Dashboard = () => {
  ...
  const handleSearchChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (term === '') {
      ...
      setFilteredCities(
        cities.filter((city) => city.city.toLowerCase().includes(term.toLowerCase())),
      );
    }
  };
  ...
  const populationData = {
    ...
    datasets: [
      {
        label: 'Population',
        data: filteredCities.map((city) => parseInt(city.population.replace(/[^\d]/g, ''))),
        ...
        <ul className="w-full max-w-4xl">
          {filteredCities.map((city) => (
            <li key={city.id} className="mb-4">
              <Link
                href={`/dashboard/${encodeURIComponent(city.city.replace(/ /g, '-'))}`}
